<ul class="nav nav-sidebar">
    <li><a href="{!! url('dashboard') !!}"><span class="fa fa-dashboard"></span> Dashboard</a></li>
    <li><a href="{!! url('user/settings') !!}"><span class="fa fa-user"></span> Impostazioni</a></li>
    {{--<li><a href="{!! url('teams') !!}"><span class="fa fa-users"></span> Team</a></li>--}}
    <li class="sidebar-header"><span>Admin</span></li>
    <li><a href="{!! url('admin/users') !!}"><span class="fa fa-users"></span> Utenti</a></li>
    <li><a href="{!! url('admin/roles') !!}"><span class="fa fa-lock"></span> Ruoli</a></li>
    <li><a href="{!! url('admin/agencies') !!}"><span class="fa fa-lock"></span> Agencies</a></li>
    <li><a href="{!! url('admin/routes') !!}"><span class="fa fa-lock"></span> Routes</a></li>
    <li><a href="{!! url('admin/shapes') !!}"><span class="fa fa-lock"></span> Shapes</a></li>
    <li><a href="{!! url('admin/fares') !!}"><span class="fa fa-lock"></span> Fare</a></li>
    <li><a href="{!! url('admin/stops') !!}"><span class="fa fa-lock"></span> Stop</a></li>
</ul>